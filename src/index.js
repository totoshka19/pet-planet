const API_URL = 'https://flicker-lean-hardhat.glitch.me';
...
const cartItemsList = modalOverlay.querySelector('.modal__cart-items');
const modalCloseButton =  modalOverlay.querySelector('.modal-overlay_close-button');
const cartTotalPriceElement = modalOverlay.querySelector('.modal__cart-price');
const cartForm = modalOverlay.querySelector('.modal__cart-form');
...
const orderMessageTCloseButton= document.createElement('button');
orderMessageTCloseButton.classList.add('order-message__close-button');
orderMessageTCloseButton.textContent = 'Закрыть';

orderMessageElement.append(orderMessageText, orderMessageTCloseButton);

orderMessageTCloseButton.addEventListener('click', () => {
...
const submitOrder = async (e) => {
  ...
  try {
    ...
    let tomorrowFormatted = tomorrow.toLocaleDateString('ru-RU', { day: 'numeric', month: 'long' });

    orderMessageText.textContent = `Ваш заказ оформлен, номер ${orderId}. Вы можете его забрать завтра ${tomorrowFormatted} после 12:00`;
